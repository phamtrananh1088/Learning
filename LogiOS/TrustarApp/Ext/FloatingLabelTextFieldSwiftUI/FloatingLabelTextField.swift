...
public struct FloatingLabelTextField: View {
    ...
    var centerTextFieldView: some View {
        ZStack(alignment: notifier.textAlignment.getAlignment()) {
            ...
            if (notifier.isAnimateOnFocus ? (!isSelected && textFieldValue.isEmpty) : textFieldValue.isEmpty) {
                ...
                .padding(.leading, notifier.textPaddingLeading)
            }
    ...
    var bottomLine: some View {
        if textFieldValue.isEmpty || !notifier.isShowError {
            return Rectangle()
                .fill(self.isSelected ? notifier.selectedLineColor : notifier.lineColor)
                .frame(height: self.isSelected ? notifier.selectedLineHeight : notifier.lineHeight, alignment: .leading)

        } else {
            return Rectangle()
                .fill((self.currentError.condition) ? (self.isSelected ? notifier.selectedLineColor : notifier.lineColor) : notifier.errorColor)
                .frame(height: self.isSelected ? notifier.selectedLineHeight : notifier.lineHeight, alignment: .leading)
        }
    }
...
extension FloatingLabelTextField {
    ...
    /// Sets the text padding leading.
    public func setTextPaddingLeading(_ leadingPadding: CGFloat) -> Self {
        notifier.textPaddingLeading = leadingPadding
        return self
    }
}
...
extension FloatingLabelTextField {
    ...
    /// Sets the show/hide search icon
    public func showSearchIcon(_ isShowSearchIcon: Bool) -> Self {
        notifier.isShowSearchIcon = isShowSearchIcon
        return self
    }
}
