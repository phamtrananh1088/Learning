...
struct UserSelectionUIView: View {
    @ObservedObject var homeVm: HomeViewModel
    @ObservedObject var roomUserSelectionVm: RoomUserSelectionVM
    @ObservedObject var addRoomVm: AddRoomVM
    ...
    private var title: String {
        if listChecked.count == 0 {
            return Resources.create_talk_room
        } else {
    ...
    private var creatRoomState: LoadingStateEnum {
        switch addRoomVm.stateLiveData {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 0) {
                ...
                HStack {
                    ...
                    if listChecked.count > 0 {
                        Button(
                            action: {
                                homeVm.addRoomVm.addRoom(users: listChecked.map({ UserLite(userId: $0.user.userId, companyCd: $0.user.companyCd)}))
                            },
                            label: {
            ...
            PopUpWindow(popupVm: addRoomVm.popupVm, show: $addRoomVm.isShowingAlert)
        }
...
//struct UserSelectionUIView_Previews: PreviewProvider {
