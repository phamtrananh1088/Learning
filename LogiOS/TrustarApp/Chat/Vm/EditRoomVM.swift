...
class EditRoomVM: BaseViewModel, ObservableObject {
        ...
        $stateLiveData
            ...
            .sink(receiveValue: { state in
                ...
                case .Value(value: _):
                    break
                case .Error(error: _):
                    self.showPopupView(popupVm: PopupViewModel(isShowTitle: false, title: "", message: Resources.room_add_user_err_alert_msg, isShowLeftBtn: false, leftBtnText: "", isShowRightBtn: true, rightBtnText: Resources.ok))
                }
            }).store(in: &bag)
    }
    ...
    func editRoom(roomId: String, users: [UserLite]) {
        saveState = .Value(value: .Loading)
        chatRoomRepository!.editRoom(roomId: roomId, users: users)
            .receive(on: DispatchQueue.main)
