...
fun File.deleteQuickly(): Boolean {
    ...
    val ok = renameTo(tmp)
    if (ok) {
        Const.ioExecutor.execute { tmp.deleteRecursively() }
    } else {
        deleteRecursively()
    }
    return ok
...
inline fun Disposable.setTo(disposableRef: AtomicReference<Disposable>) {
    disposableRef.replace(this)
}
...
inline fun <T : Any> Flowable<T>.blockingFirstOrNull(): T? {
    return firstElement().blockingGet()
}
...
fun Context.openMap(address: String) {
    val query = Uri.parse("geo:0,0?q=" + Uri.encode(address))
...
fun TimeZone.unixTimeDaysBetween(from: Long, to: Long): Int {
...
val File.extensionIncludeDot: String
    get() = extension.run { if (isEmpty()) "" else ".$this" }
...
inline fun <T, reified R> List<T>.mapToArrayList(transform: (T) -> R): ArrayList<R> {
...
fun <R> castArrayToList(size: Int): io.reactivex.rxjava3.functions.Function<in Array<*>, out List<R>> {
    return io.reactivex.rxjava3.functions.Function { arr ->
...
fun <V : Any?> ListenableFuture<V>.getSuccessResult(executor: Executor, action: (V) -> Unit) {
    addListener({
        runCatching { get() }.onSuccess(action)
    }, executor)
}

fun <T> MutableList<T>.lastRemove(element: T) {
...
fun checkerboard(
    size: Int,
    color1: Int = 0xff666666.toInt(),
    color2: Int = 0xff999999.toInt(),
): Shader {
    val sf = size.toFloat()
    val paint = android.graphics.Paint().apply {
        style = android.graphics.Paint.Style.FILL
    }
    val bitmap = Bitmap.createBitmap(size * 2, size * 2, Bitmap.Config.RGB_565)
    paint.color = color1
    val canvas = android.graphics.Canvas(bitmap)
    canvas.drawRect(0F, 0F, sf, sf, paint)
    canvas.translate(sf, sf)
    canvas.drawRect(0F, 0F, sf, sf, paint)
    paint.color = color2
    canvas.translate(0F, -sf)
    canvas.drawRect(0F, 0F, sf, sf, paint)
    canvas.translate(-sf, sf)
    canvas.drawRect(0F, 0F, sf, sf, paint)
    val m = Shader.TileMode.REPEAT
    return BitmapShader(bitmap, m, m)
}

fun BigDecimal.stripTrailingZerosString(): String {
    return stripTrailingZeros().toPlainString()
}

fun Double.stripTrailingZerosString(): String {
    return if (this == 0.0) "0" else BigDecimal(toString()).stripTrailingZerosString()
}

fun <T : Any, R : Any> Flowable<Optional<T>>.optionalSwitchMap(switch: (T) -> Flowable<R>): Flowable<R> {
    return switchMap {
        val v = it.orElseNull()
        if (v == null) Flowable.empty() else switch(v)
    }
}

fun <T : Any, R : Any> Flowable<Optional<T>>.switchMapOptionalOrEmpty(switch: (T) -> Flowable<Optional<R>>): Flowable<Optional<R>> {
    return switchMap {
        val v = it.orElseNull()
        if (v == null) Flowable.just(Optional.empty()) else switch(v)
    }
}

fun <T : Any> Flowable<Optional<T>>.switchMapIfOptionalEmpty(ifEmpty: () -> Flowable<Optional<T>>?): Flowable<Optional<T>> {
    return switchMap {
        val v = it.orElseNull()
        if (v == null) ifEmpty() ?: Flowable.just(Optional.empty()) else Flowable.just(it)
    }
}

fun <T : Any, R : Any> Flowable<Optional<T>>.mapOptionalOrEmpty(switch: (T) -> R?): Flowable<Optional<R>> {
    return map {
        Optional.ofNullable(it.orElseNull()?.let(switch))
    }
}

fun <T : Any> Flowable<Optional<T>>.distinctUntilOptionalChanged(): Flowable<Optional<T>> {
    return distinctUntilChanged { t1, t2 -> t1.orElseNull() == t2.orElseNull() }
}

fun <T : Any> List<T>.secondToLast(): T? {
    return getOrNull(lastIndex - 1)
}

fun Location.elapsedMillis(old: Location): Long {
    return (elapsedRealtimeNanos - old.elapsedRealtimeNanos) / 1000_000
}

fun <T : Any> Flowable<T>.withPrevious(): Flowable<Pair<T?, T>> {
    @Suppress("UNCHECKED_CAST")
    return scan(Pair<T?, T?>(null, null)) { t1, t2 -> t1.second to t2 }
        .skip(1) as Flowable<Pair<T?, T>>
}

fun dateBasedOnCurrentDate(realtimeNanos: Long): Long {
    val passed = (SystemClock.elapsedRealtimeNanos() - realtimeNanos) / 1000_000
    return System.currentTimeMillis() - passed
}
...
fun <T : Disposable> AtomicReference<T>.replace(disposable: T?) {
    getAndSet(disposable)?.dispose()
}

inline fun <T, V : Any> T.mapIfNotNull(value: V?, block: T.(V) -> T): T {
    return if (value != null) block(value) else this
}

fun longComparator(): Comparator<Long> = compareBy { it }

fun intComparator(): Comparator<Int> = compareBy { it }

fun String.queryStr(): String {
    val chars = toCharArray()
    chars.forEachIndexed { index, c ->
        when (c.code) {
            in 97..122 -> {//a-z
                chars[index] = c - 32//A-Z
            }

            in 12449..12534 -> {//ァ..ヶ
                chars[index] = c - 96//ぁ..ゖ
            }

            in 65296..65305 -> {//０..９
                chars[index] = c - 65248 //0..9
            }
        }
    }
    return String(chars)
...
inline fun <T, R> List<T>.fastFoldWithPrev(initial: R, op: (r: R, p: T?, e: T) -> R): R {
    var acc = initial
    var p: T? = null
...
inline fun <T> List<T>.fastForEachWithNext(block: (e: T, next: T?) -> Unit) {
    val last = lastIndex
    if (last < 0) return
    var e = get(0)
    for (i in 1..last) {
