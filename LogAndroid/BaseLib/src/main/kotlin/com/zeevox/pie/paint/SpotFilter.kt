...
class SpotFilter(
        private var mBufSize: Int,
        posDecay: Float,
        pressureDecay: Float,
        private var mPlotter: Plotter
) {
    private val spots = LinkedList<MotionEvent.PointerCoords>() // newest at front
    private val tmpSpot = MotionEvent.PointerCoords()
    private var lastTool = MotionEvent.TOOL_TYPE_UNKNOWN

    private val posDecay: Float = if (posDecay in 0f..1f) posDecay else 1f
    private val pressureDecay: Float = if (pressureDecay in 0f..1f) pressureDecay else 1f

    interface Plotter {
        fun plot(s: MotionEvent.PointerCoords)
    }

    private fun filterInto(out: MotionEvent.PointerCoords, tool: Int): MotionEvent.PointerCoords {
        lastTool = tool
    ...
    private fun addInternal(c: MotionEvent.PointerCoords, tool: Int) {
        val coord =
