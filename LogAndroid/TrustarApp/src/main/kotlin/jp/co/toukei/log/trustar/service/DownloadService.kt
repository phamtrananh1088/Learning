...
class DownloadService : Service() {
    ...
    private fun download(item: DL): Flowable<Progress> {
        return when (item) {
            is DL.KeyFile -> {
                Single.defer { Current.chatApi.download(item.fileKey) }
                    .downloadToFile(
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        startForeground(Config.nidDownload, buildNotificationGroup())
    ...
    private fun buildNotificationGroup(): Notification {
        return buildNotification(Config.NotificationChannelIdDownload) {
