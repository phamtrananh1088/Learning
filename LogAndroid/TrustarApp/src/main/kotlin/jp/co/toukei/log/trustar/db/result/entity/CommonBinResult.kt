...
class CommonBinResult(
    ...
    override fun jsonBody(): JSONObject {
        return jsonObj {
            ...
            startLocation?.apply {
                "startLatitude" v (latitude ?: 0.0)
                "startLongitude" v (longitude ?: 0.0)
                "startAccuracy" v (accuracy ?: 0F)
                "startDatetime" v (date)
            }
            endLocation?.apply {
                "endLatitude" v (latitude ?: 0.0)
                "endLongitude" v (longitude ?: 0.0)
                "endAccuracy" v (accuracy ?: 0F)
                "endDatetime" v (date)
            }
