...
class ChatRoomRepository {
    ...
    fun leaveRoom(roomId: String): Single<Optional<String>> {
        val setting = ChatUserSettings(roomId, Current.userId, null, true)
        return api.updateUserSetting(setting)
            .observeOnIO()
            .toState()
            .doOnSuccess { Current.chatDB.getInstance().chatRoomDao().deleteByRoomId(roomId) }
    }

    fun notificationOnOff(roomId: String, on: Boolean): Single<Optional<String>> {
        val setting = ChatUserSettings(roomId, Current.userId, on, null)
        return api.updateUserSetting(setting)
            .observeOnIO()
            .toState()
    }
    ...
    fun addRoom(users: List<UserLite>): Completable {
        return api.addRoom(RoomCreate(users))
            .observeOnIO()
            .toCompletable()
    }

    fun editRoom(roomId: String, users: List<UserLite>): Completable {
