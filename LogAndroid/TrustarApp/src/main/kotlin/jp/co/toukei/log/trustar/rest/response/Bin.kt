...
class Bin @Keep constructor(
    ...
    @JvmField
    val collectionResults: List<CollectionResult>? = collections?.groupBy { Pair(it.allocationNo, it.allocationRowNo) }
            ?.map { (k, v) ->
                CollectionResult(k.first, k.second, v.map {
        ...
        val binDetail: BinDetail = run {
            ...
            val atf = wd?.let { date ->
                appointedTimeFrom?.hms60From4dig()?.let { min ->
                    Config.GMT9.calendar(date.time)
                            .apply { setMidnight0() }
            ...
            BinDetail(
                    ...
                    allocationRowNo,
                    allocationPlanFlag,
                    if (workCd != null) BinWork(workCd, workNm) else null,
                    serviceOrder,
                    atf,
                    att,
                    Place(placeCd, placeNm1, placeNm2, placeAddr),
                    PlaceExt(
                            placeZip,
                    ...
                    workEnd,
                    temperature,
                    experiencePlaceNote1,
                    updatedDate?.dateFromString2()?.time
            )
        }
    }
