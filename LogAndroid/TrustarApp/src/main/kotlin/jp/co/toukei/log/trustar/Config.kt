...
import jp.co.toukei.log.trustar.rest.UserApi
...
object Config {
    ...
    @JvmField
    val apiKey = Ctx.context.assets.open("app.properties").use {
        val p = Properties().apply { load(it) }
        p.getProperty("API_KEY") ?: throw AssertionError("apiKey error")
    }
    ...
    @JvmField
    val GMT9: TimeZone = TimeZone.getTimeZone("GMT+9")
    ...
    val locationLogFile: File
        get() = cacheDir.child("location_log")
    ...
    const val LocationSettingsNotificationId = 7
    const val LocationNotificationChannelId = "l_id"
    const val LocationNotificationId = 6
    const val SettingsNotificationChannelId = "p_id"
    const val PowerSaverSettingsNotificationId = 5
    const val downloadNotificationChannelId = "download_id"
    const val downloadNotificationId = 4
    const val messageNotificationChannelId = "message_id"

    const val ACTION_HTTP_401 = "ACTION_HTTP_401"
    ...
    private val noCacheHttpClient: Call.Factory = OkHttpClient.Builder()
        ...
        .addNetworkInterceptor { chain ->
            chain.proceed(chain.request()).also {
                if (it.code == 401) {
                    Ctx.context.localBroadcastManager.sendBroadcast(Intent(ACTION_HTTP_401))
                }
            }
        }
        .apply {
    ...
    @JvmField
    val fetchApi: FetchApi = retrofit.create()

    @JvmField
    val syncApi: SyncApi = retrofit.create()
    ...
    @JvmField
    val dateFormatter = ReuseDateFormatter("yyyy-MM-dd'T'HH:mm:ss", locale, GMT9)

    @JvmField
    val dateFormatter2 = ReuseDateFormatter("yyyy/MM/dd HH:mm:ss", locale, GMT9)

    @JvmField
    val dateFormatter3 = ReuseDateFormatter("yyyy/MM/dd HH:mm", locale, GMT9)

    @JvmField
    val dateFormatterMMddHHmm = ReuseDateFormatter("MM月dd日 HH:mm", locale, GMT9)

    @JvmField
    val mmddFormatter = ReuseDateFormatter("MM/dd", locale, GMT9)

    @JvmField
    val dateFormatterMMdde = ReuseDateFormatter("MM月dd日（E）", locale, GMT9)

    @JvmField
    val timeFormatter = ReuseDateFormatter("yyyyMMddHHmmss", locale, GMT9)

    @JvmField
    val dateFormatterForChat = ReuseDateFormatter("yyyy年M月d日 E曜日 HH:mm", locale, GMT9)
