...
class Recorder : ToolbarActivity() {
    ...
    companion object {
        ...
        fun intentForStartActivity(context: Context, filePath: File): Intent {
            return context.intentFor<Recorder>(
                TARGET_FILE to filePath.absolutePath,
            )
        }

        fun resolveResult(result: ActivityResult): Result? {
            if (result.resultCode != Activity.RESULT_OK) return null
            val data = result.data ?: return null
            val uri = data.getStringExtra(TARGET_FILE)?.toUri() ?: return null
            val ext = data.getStringExtra(TARGET_FILE_EXT)
            return Result(uri, ext)
        }
