...
import jp.co.toukei.log.trustar.repo.LocalFileRepo
...
class ChatVM : CommonViewModel() {
    ...
    private val fr = LocalFileRepo(Current.userInfo)
            ...
            .flatMapCompletable(true, 1) { tmp ->
                ...
                Completable
                    .fromCallable {
                        ...
                        fr.saveLocalAttachment(t2, name) { att ->
                            val file = att.file
    ...
    fun sendAudio(roomId: String, uri: Uri, ext: String?) {
        ...
        Completable
            .fromCallable {
                ...
                fr.saveLocalAttachment(tmp, name) {
                    val message = ChatMessagePending.audioMessage(roomId, it)
                    Current.chatDB.getInstance().chatMessagePendingDao().insert(message)
                }
            }
            .upload()
    ...
    fun sendVideo(roomId: String, uri: Uri, extension: String) {
        ...
        Completable
            .fromCallable {
                ...
                fr.saveLocalAttachment(tmp, name) {
                    val message = ChatMessagePending.videoMessage(roomId, it)
                    Current.chatDB.getInstance().chatMessagePendingDao().insert(message)
                }
            }
            .upload()
    ...
    fun sendFile(roomId: String, uri: Uri) {
        ...
        Completable
            .fromCallable {
                fr.saveLocalAttachment(tmp, name ?: "unknown") {
                    val message = ChatMessagePending.fileMessage(roomId, it)
                    Current.chatDB.getInstance().chatMessagePendingDao().insert(message)
                }
            }
            .upload()
