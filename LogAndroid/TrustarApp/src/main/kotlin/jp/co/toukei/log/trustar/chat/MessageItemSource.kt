        ...
        list.forEachPrevNext { prev, it, next ->
            ...
            target += when (it.type) {
                ChatMessage.TYPE_VIDEO -> {
                    MessageItem.Sent.VideoSent(uid, it, userGetter(it.userId), readStatus(it), p, n)
                }

                ChatMessage.TYPE_AUDIO -> {
                    MessageItem.Sent.AudioSent(uid, it, userGetter(it.userId), readStatus(it), p, n)
                }

                ChatMessage.TYPE_IMG -> {
                    MessageItem.Sent.ImgSent(uid, it, userGetter(it.userId), readStatus(it), p, n)
                }

                ChatMessage.TYPE_FILE -> {
                    MessageItem.Sent.FileSent(uid, it, userGetter(it.userId), readStatus(it), p, n)
                }

                else -> MessageItem.Sent.TextSent(uid, it, userGetter(it.userId), readStatus(it), p, n)
            }
        }
        return target
    }
        ...
        list.forEachPrevNext { prev, it, next ->
            ...
            target += when (it.type) {
                ChatMessagePending.TYPE_IMG_SEND, ChatMessagePending.TYPE_IMG -> {
                    MessageItem.Pending.ImgPending(uid, it, p, n)
                }

                ChatMessagePending.TYPE_AUDIO_SEND, ChatMessagePending.TYPE_AUDIO -> {
                    MessageItem.Pending.AudioPending(uid, it, p, n)
                }

                ChatMessagePending.TYPE_VIDEO_SEND, ChatMessagePending.TYPE_VIDEO -> {
                    MessageItem.Pending.VideoPending(uid, it, p, n)
                }

                ChatMessagePending.TYPE_FILE_SEND, ChatMessagePending.TYPE_FILE -> {
                    MessageItem.Pending.FilePending(uid, it, p, n)
                }

                else -> MessageItem.Pending.TextPending(uid, it, p, n)
            }
        }
        return target
    }
