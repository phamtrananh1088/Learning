...
abstract class UserSelectionVM<T : Member> : CommonViewModel() {

    class UserCheck<out T : Member>(
        @JvmField val user: T,
        @JvmField val checked: Boolean,
        @JvmField val keepState: Boolean,
    )

    protected open fun keepState(t: T): Boolean = false

    private val currentUser = BehaviorProcessor.create<List<UserCheck<T>>>()

    protected val fetch: BehaviorProcessor<Flowable<out List<List<T>>>> = BehaviorProcessor.create()
            ...
            g.map { l ->
                l.mapNotNull {
                    if (!e.contains(it.userId)) {
                        ...
                        UserCheck(it, s.contains(it.userId), f?.keepState ?: keepState(it))
                    } else null
                }
            }
        }
    ...
    private val checkedResult: Flowable<List<UserCheck<T>>> = Flowable
        .combineLatest(
            merge,
            selectedDirtyPool.observeOnComputation()
        ) { l, s ->
    ...
    @JvmField
    val loadStateLiveData: LiveData<out Result<Any>> = groups.toLiveData()

    @JvmField
    val topList: LiveData<List<UserCheck<T>>> = checkedResult.toLiveData()

    @JvmField
    val displayList: LiveData<List<List<UserCheck<T>>>> = lastList
        .switchMap { g ->
            g.mapToArray {
                Flowable.just(it).filterListByQuery(queryText, 100) { e, w ->
                    e.user.userName?.contains(w, true) ?: false
                }
            }.combineLatest()
        }
        .replayThenAutoConnect(disposableContainer)
    ...
    private val keep = BehaviorProcessor.createDefault<List<UserCheck<T>>>(emptyList())

    init {
        merge.subscribe(keep)
    }

    private val selectedIds = ArraySet<String>()
    ...
    fun selectId(id: String, select: Boolean) {
        val f = keep.value?.firstOrNull { it.user.userId == id }
        if (f != null && !f.keepState) {
