...
interface BinDetailDao : CommonDao<BinDetail> {
    ...
    @Query("select * from bin_detail where allocation_no = :allocationNo and allocation_row_no = :allocationRowNo")
    fun selectBinDetail(allocationNo: String, allocationRowNo: Int): Flowable<Optional<BinDetail>>
    ...
    @Query("update bin_detail set sync = $SyncPending where sync = $SyncChanged")
    fun setPending()

    @Query("select * from bin_detail where sync = $SyncPending")
    fun getPending(): List<BinDetail>

    @Query("select * from bin_detail where sync = $SyncDone")
