...
class CommonWorkResult(
    ...
    override fun jsonBody(): JSONObject {
        return jsonObj {
            ...
            workStart?.apply {
                "latitudeFrom" v (location?.lat ?: 0F)
                "longitudeFrom" v (location?.lon ?: 0F)
                "accuracyFrom" v (location?.accuracy ?: 0F)
                "workDatetimeFrom" v date
            }
            workEnd?.apply {
                "latitudeTo" v (location?.lat ?: 0F)
                "longitudeTo" v (location?.lon ?: 0F)
                "accuracyTo" v (location?.accuracy ?: 0F)
