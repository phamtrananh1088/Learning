...
import jp.co.toukei.log.lib.rxBiFunctionTakeLeft
...
class RoomLoadVM : CommonViewModel() {
    ...
    private val lastTimeMillis = BehaviorProcessor.createDefault(System.currentTimeMillis())
    ...
    @JvmField
    val listLiveData = Flowable
        .combineLatest(
            chatRoomRepository.sortedRoomList(),
            lastTimeMillis,
            rxBiFunctionTakeLeft()
        )
        .replayThenAutoConnect(disposableContainer)
        .toLiveData()

    fun updateDate(timeMillis: Long) {
        lastTimeMillis.onNext(timeMillis)
    }
}
