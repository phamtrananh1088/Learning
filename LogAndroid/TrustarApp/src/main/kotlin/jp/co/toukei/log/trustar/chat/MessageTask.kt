...
import jp.co.toukei.log.trustar.repo.chat.ChatFileRepository
...
class MessageTask : Disposable {
    ...
    private fun <T> Maybe<T>.doOnUploadError(dao: ChatMessagePendingDao, id: Long): Maybe<T> {
        return doOnError {
                ...
                when (pending.type) {
                    ChatMessagePending.TYPE_IMG_SEND -> Maybe
                        .defer {
                            ...
                            } else fileRepository.uploadImage(f.name, f.file.asRequestBody())
                                .map { m to it }
                                .toMaybe()
                        }
                        .observeOnIO()
                    ...
                    ChatMessagePending.TYPE_AUDIO_SEND -> Maybe
                        .defer {
                            ...
                            if (m == null || f == null || !f.file.canRead()) {
                                Maybe.fromAction { repo.deletePendingMessage(pending) }
                            } else fileRepository.uploadFile(f.name, f.file.asRequestBody())
                                .map { m to it }
                                .toMaybe()
                        }
                        .observeOnIO()
                    ...
                    ChatMessagePending.TYPE_VIDEO_SEND -> Maybe
                        .defer {
                            ...
                            if (m == null || f == null || !f.file.canRead()) {
                                Maybe.fromAction { repo.deletePendingMessage(pending) }
                            } else fileRepository.uploadVideo(f.name, f.file.asRequestBody())
                                .map { Triple(m, it, f) }
                                .toMaybe()
                        }
                        .observeOnIO()
                    ...
                    ChatMessagePending.TYPE_FILE_SEND -> Maybe
                        .defer {
                            ...
                            if (m == null || f == null || !f.file.canRead()) {
                                Maybe.fromAction { repo.deletePendingMessage(pending) }
                            } else fileRepository.uploadFile2(f.name, f.file.asRequestBody())
                                .map { Triple(m, it, f) }
                                .toMaybe()
                        }
                        .observeOnIO()
